...
public var numberOfPickups:int = 4;
...
function Awake()
{
	...
	textfield = GameObject.Find("Score").GetComponent(GUIText);
	score = 0;
	UpdateScoreText();
...
function Collected(pickupCollected:GameObject)
{
	...
	var index:int = parseInt(pickupCollected.transform.parent.gameObject.name);
	...
	{
		GameObject.Find("DW").GetComponent(DWController).walkSpeed += 10;
	}
	else if (type == 3)
	{
		GameObject.Find("DW").GetComponent(DWController).walkSpeed -= 5;
	}
	...
	// destroy the pickup
	Destroy(pickupCollected.transform.parent.gameObject);
